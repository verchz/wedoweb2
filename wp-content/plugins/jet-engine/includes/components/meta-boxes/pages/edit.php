	...
	class Jet_Engine_Meta_Boxes_Page_Edit extends Jet_Engine_CPT_Page_Base {
		...
		public function page_specific_assets() {
			...
			wp_enqueue_script(
				...
				jet_engine()->plugin_url( 'assets/js/admin/meta-boxes/delete-dialog.js' ),
				array( 'cx-vue-ui', 'wp-api-fetch', ),
				jet_engine()->get_version(),
				true
			);
			...
			wp_enqueue_script(
				'jet-engine-mb-edit',
				jet_engine()->plugin_url( 'assets/js/admin/meta-boxes/edit.js' ),
				array( 'cx-vue-ui', 'wp-api-fetch' ),
				jet_engine()->get_version(),
				true
			);
			...
			wp_localize_script(
				...
				$this->manager->get_admin_page_config( array(
					...
					'redirect'          => $redirect,
					'sources'           => apply_filters( 'jet-engine/meta-boxes/sources', array(
						array(
							'value' => 'post',
		...
		public function add_page_template() {
			...
			ob_start();
			include jet_engine()->get_template( 'admin/pages/meta-boxes/edit.php' );
			$content = ob_get_clean();

			printf( '<script type="text/x-template" id="jet-mb-form">%s</script>', $content );

			ob_start();
			include jet_engine()->get_template( 'admin/pages/meta-boxes/delete-dialog.php' );
			$content = ob_get_clean();
