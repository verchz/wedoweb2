Vue.component( 'jet-meta-fields', {
	...
	computed: {
		...
		fieldsOptionList: function() {
			var result = [],
				blackTypesList = [ 'repeater', 'media', 'gallery', 'posts', 'iconpicker' ];
	...
	methods: {
		...
		sanitizeFieldName: function( index ) {
			...
			var regex = /\s+/g;
			field.name = field.name.toLowerCase().replace( regex, '-' );
			field.name = this.maybeCyrToLatin( field.name );
		...
		sanitizeRepeaterFieldName: function( fieldIndex, repeaterFieldIndex ) {
			...
			var regex = /\s+/g;
			repeaterField.name = repeaterField.name.toLowerCase().replace( regex, '-' );
			repeaterField.name = this.maybeCyrToLatin( repeaterField.name );
